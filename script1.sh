...
cnt=$(grep '^write' $1 | wc -l)

echo "N. di write: " $cnt 

grep write $1 > $tmpfile 
sum=0
exec 3<$tmpfile    # apro il file in lettura e gli assegno il descrittore 3 
while IFS=" " read -u 3 line; do #leggo riga per riga
...
echo "Numero volte che è stato necessario l'algoritmo di rimpiazzamento: " $cnt  # l'opzione -e ci consente di usare simboli speciali come \t (tab)

cat $1 | grep -v 'read' | grep -v 'write' | grep -v 'rimpiazzamento' | grep -v "connessione" | cut -d" " -f 5  | sort -g | uniq -c > $tmpfile
...
cat $1 | grep "open_c\|open_cl\|remove" | cut -d" " -f 1 > $tmpfile
...
exec 3<&-  # chiudo il descrittore 3 
echo "Numero massimo di file memorizzati: " $cnt_max
...
cat $1 | grep "write\|append\|remove\|rimpiazzamento" | cut -d" " -f 1,3,4 > $tmpfile
...
exec 3<&-  # chiudo il descrittore 3 
echo "Dimensione massima in Mbytes raggiunta dallo storage: " $cnt_max
